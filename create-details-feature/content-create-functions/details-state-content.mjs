export function stateContent(entityName) {
  ...
  > {
    ...
    @Action(SearchByEntityCode, { cancelUncompleted: true })
    searchByEntityCode(ctx: StateContext<${entityName}StateModel>, action: SearchByEntityCode): Observable<${entityName}SearchResponse> {
      const state = ctx.getState();
      ctx.patchState({
        filterForm: {
          ...state.filterForm,
          model: {
            ...state.filterForm.model,
            xxx: action.entityCode,
          },
        },
      });
      return super.searchAction(ctx, action).pipe(
        finalize(() => {
          ctx.patchState({
            filterForm: {
              ...getBaseDefaultState().filterForm,
            },
          });
        })
      );
